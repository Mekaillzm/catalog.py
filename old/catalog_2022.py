...
class catalogue():
    def __init__(self):
        ...
        try:
            self.pref = pd.read_csv('preferences.csv', index_col=0)['0']
        except:
            self.pref = pd.Series({'firstTimeSetup': False})
            self.pref.to_csv('preferences.csv')
            self.fileName = ''
        ...
        try:
            self.df[self.columns[3]] = list(
                map(pd.to_datetime, self.df[self.columns[3]]))
        except:pass
        ...
        self.infoFrame = Toplevel(master=self.mainWin)
        self.infoFrame.title('Information')
        ...
        self.infoButton = Button(
            master=self.searchFrame, command=self.infoFrame.deiconify, text='i', width=1)
        ...
        self.selectPageButton = Checkbutton(
            master=self.resultFrames[0], text='📄', command=self.onSelectPage, var=self.pageSelected)

        self.resultLabels = [
            Label(text=column, master=self.resultFrames[0]) for column in self.df.columns]
        self.resultEntries = []
        ...
        for row in range(self.numResults):
            ...
            for n in range(len(self.df.columns)):
                ...
                self.resultEntries[row].append(
                    Entry(master=self.resultFrames[row], width=width))
        # pageFrame
        ...
        self.bookCombobox = Combobox(
            master=self.infoFrame, state='readonly', width=sw)
        ...
        self.infoFrame.withdraw()
        # No result frame
        self.noResultLabel = Label(master=self.noResultFrame, text="No result found, try searching using other relevant information like the author's name", font=(
        ...
        self.selectReplaceLabels = [
            Label(master=self.selectReplaceFrame) for column in self.df.columns]
        ...
        self.editMenu.add_command(
            label='Replace', command=self.advancedSelectWin.deiconify)
        ...
        self.mainWin.after(500, lambda: self.searchFrame.grid(
            row=0, column=1, pady=40))

        self.addButton.grid(row=0, column=1, ipady=3)
        ...
        self.resultFrame.grid(row=2, column=1, padx=25)
        ...
        self.mapList = lambda l, row, minCol=0: [
            l[n].grid(row=row, column=n+minCol) for n in range(len(l))]
        ...
        self.infoFrame.bind('<<ComboboxSelected>>', self.infoDisplay)
        ...
        self.infoFrame.protocol("WM_DELETE_WINDOW", self.infoFrame.withdraw)
        self.mainWin.protocol("WM_DELETE_WINDOW", self.onClose)

        self.mainWin.bind('<<ComboboxSelected>>', self.cbSelect)
        self.mainWin.after(1, self.search)
        self.mainWin.after(2, self.updateInfoFrame)
        

        self.mainWin.mainloop()
    
    def newDf(self, userInput=True, openAdd=False):
        try:
            self.update()
            self.df.to_csv(self.fileName)
    ...
    def updateText(self):
        try:
            ...
            self.textDf=pd.DataFrame({'id':['columns','itemName','systemName','action1','state1','action2','state2','columnDTypes','lastColumnAndItemRelation','secondColumnAndItemRelation','defaultColumnValues'],
                                      'text':['Title♣Author♣Type♣Date Added♣Available♣Borrower♣Borrow Date♣Due Date♣Times Borrowed♣Donor','book','catalog','borrow','borrowed','available','available','str♣str♣str♣date♣str♣str♣date♣date♣int♣str','donated by','written by','Unknown♣Unknown♣Unknown♣today♣no♣Unknown♣today♣NaT♣1♣Unknown']}).set_index('id')
        ...
        except:self.textDf.loc['itemName']='book'
        
        try:self.systemName=self.textDf.loc['systemName'].values[0]
        except:self.textDf.loc['systemName']='catalog'
        try:self.columns=self.textDf.loc['columns'].values[0].split('♣')
        except:self.textDf.loc['columns']='Title♣Author,Type♣Date Added♣Available♣Borrower♣Borrow Date♣Due Date♣Times Borrowed♣Donor'
        try:self.action1=self.textDf.loc['action1'].values[0]
        except:
            self.textDf.loc['action1']='borrow'
            self.action1='borrow'
        try:self.state1=self.textDf.loc['state1'].values[0]
        except:
            self.textDf.loc['state1']='borrowed'
            self.state1='borrowed'
        try:self.state2=self.textDf.loc['state2'].values[0]
        except:
            self.textDf.loc['state2']='available'
            self.state2='available'
        try:self.action2=self.textDf.loc['action2'].values[0]
        except:
            self.textDf.loc['action2']='available'
            self.action2='available'
        
        try:self.columnDTypes=self.textDf.loc['columnDTypes'].values[0].split('♣')
        except:
            self.textDf.loc['columnDTypes']='str♣str♣str♣date♣str♣str♣date♣date♣int♣str'
            self.columnDTypes=['str', 'str', 'str', 'date', 'str', 'str', 'date', 'date', 'int', 'str']
        try:
            self.columnDefaults=self.textDf.loc['defaultColumnValues'].values[0].split('♣')
        except:
            self.textDf.loc['defaultColumnValues']='Unknown♣Unknown♣Unknown♣today♣no♣Unknown♣today♣NaT♣1♣Unknown'
            self.columnDefaults=['Unknown','Unknown','Unknown','today','no','Unknown','today','NaT','1','Unknown']

        self.textDf.to_csv(self.fileName[:-4]+'-text.csv')

        self.addFrame.title(f'Add {self.itemName.capitalize()}')
       
        self.askBorrower.title(f'Enter {self.columns[5]} and {self.columns[7]}')
        ...
        self.bookUniqueNumLabel.config(text=f'Unique {self.columns[0]}(s)')
        self.bookNumLabel.config(text=f'Number of {self.columns[0]}(s)')       
        ...
        self.askDueDateLabel.config(text=f"What's this {self.itemName}'s {self.columns[6]}?")
        ...
        [self.selectReplaceLabels[n].config(text=self.columns[n]) for n in range(len(self.columns))]
    ...
    def openDf(self):
        ...
        success = False
        while success != True:
            file = filedialog.askopenfile(master=self.mainWin, title=f'Select a {self.itemName} file', filetypes=(
    ...
    def confirmDataIntegrity(self,new=False):
        ...
        if self.df.shape[1]!=10 or new==True:
            dfCols=['Title',
                     ...
                     'Donor']
            dfCols2=dfCols.copy()
            _=msg.showinfo(master=self.mainWin,title='Enter column names',message=f"Go through a few simple steps to name each column of your data")
            if _=='ok':
                self.sortColumnsN=-1
                skippedCols=[]
                ...
                columnTypes={}
                def displaySet():
                    if self.sortColumnsN>=10:
                        self.sortColumnsWin.destroy()
                        end()
                        return
                    col=dfCols2[self.sortColumnsN]
                    self.sortColumnsColumnLabel.config(text=col)
                    self.sortColumnsLabel.config(text=f"What information does the '{col}' column contain?")
                    
                    try:
                        self.sortColumnsSampleCombobox['values']=self.df[col].to_list()
                        self.sortColumnsSampleCombobox.set(str(self.df.loc[sample,col].values[0]))
                    except:pass
                    self.sortColumnsTypeCombobox.set('')

                    self.sortColumnsCombobox.focus_set()
                    
                def nextCol():
                    
                    if self.sortColumnsN>-1:
                        self.df2[self.sortColumnsCombobox.get()]=self.df[self.df.columns[self.sortColumnsN]].to_list()
                        columnTypes[self.sortColumnsCombobox.get()]=self.sortColumnsTypeCombobox.get()
                        
                        self.sortColumnsBackButton.config(state='normal')
                        try:dfCols.pop(dfCols.index(self.sortColumnsCombobox.get()))
                        except:pass
                        self.sortColumnsCombobox['values']=dfCols
                        self.sortColumnsCombobox.set('')
                    self.sortColumnsN+=1

                    displaySet()                    
                def prevCol():
                    self.sortColumnsN-=1
                    if self.sortColumnsN in skippedCols:
                        skippedCols.pop(-1)
                    else:
                        dfCols.append(self.df2.columns[-1])
                        del self.df2[self.df2.columns[-1]]
                        self.sortColumnsCombobox['values']=dfCols
                        self.sortColumnsCombobox.set('')
                        
                    if self.sortColumnsN<1:
                        self.sortColumnsBackButton.config(state='disabled')
                    columnTypes.pop(list(columnTypes.keys())[-1])
                    displaySet()
                def skipCol():
                    skippedCols.append(self.sortColumnsN)
                    self.sortColumnsN+=1
                    if self.sortColumnsN>0:
                        self.sortColumnsBackButton.config(state='normal')
                    displaySet()
                def end():
                    for n in range(len(self.df2.columns),10):
                        self.df2[n]=[float('nan')]*len(self.df2)
                    self.df=self.df2.copy()
                    del self.df2
                    self.columns=self.df.columns
                    [self.resultLabels[n].config(text=self.columns[n]) for n in range(len(self.df.columns))]
                    #self.pref[f'itemName~{self.fileName}']='book'
                    #self.pref[f'systemName~{self.fileName}']='catalog'
                    
                    
                    #self.pref[f'columnTypes~{columnTypes}']
                    #try:
                    #    self.itemName=self.pref[f'itemName~{self.fileName}']
                    #    self.systemName=self.pref[f'itemName~{self.systemName}']
                    #except:pass
                    ...
                    try:
                        ...
                        self.search(False,True,True)
                        self.mainWin.after(10,self.update)
                def sortKeyPress(i):
                    if i.keysym in ['Enter','Return']:
                        nextCol()
                    
                self.sortColumnsWin=Toplevel(master=self.mainWin)
                self.sortColumnsLabel=Label(master=self.sortColumnsWin,text='What information does this column contain?')
                
                self.sortColumnsFrame1=Frame(master=self.sortColumnsWin)
                self.sortColumnsColumnLabel=Label(master=self.sortColumnsFrame1,relief='groove',width=20)
                self.sortColumnsCombobox=Combobox(master=self.sortColumnsFrame1,state='normal')
                self.sortColumnsCombobox['values']=self.columns
                self.sortColumnsCombobox.set('')
                
                self.sortColumnsTypeLabel=Label(master=self.sortColumnsFrame1,text='Data Type',width=12)
                self.sortColumnsTypeCombobox=Combobox(master=self.sortColumnsFrame1,state='readonly',width=10)
                self.sortColumnsTypeCombobox['values']=['Text','Number','Date']
                self.sortColumnsTypeCombobox.set('Text')
                
                self.sortColumnsRLabel=Label(master=self.sortColumnsFrame1,text='is')
                self.sortColumnsSampleLabel=Label(master=self.sortColumnsFrame1,text='Sample Data',width=12)
                self.sortColumnsSampleCombobox=Combobox(master=self.sortColumnsFrame1,state='readonly',width=10)
                
                try:sample=self.df.sample().index
                except:sample=None
                self.sortColumnsFrame2=Frame(master=self.sortColumnsWin)
                self.sortColumnsNextButton=Button(master=self.sortColumnsFrame2,text='Next',command=nextCol)
                self.sortColumnsSkipButton=Button(master=self.sortColumnsFrame2,text='Skip',command=skipCol)
                self.sortColumnsBackButton=Button(master=self.sortColumnsFrame2,text='Back',state='disabled',command=prevCol)
                
                
                self.sortColumnsLabel.grid(row=0,column=0)
                self.sortColumnsFrame1.grid(row=1,column=0)
                self.sortColumnsFrame2.grid(row=2,column=0)
                
                self.sortColumnsColumnLabel.grid(row=0,column=0)
                self.sortColumnsCombobox.grid(row=0,column=2)
                self.sortColumnsRLabel.grid(row=0,column=1)
                self.sortColumnsTypeLabel.grid(row=0,column=3)
                self.sortColumnsTypeCombobox.grid(row=0,column=4)
                self.sortColumnsSampleLabel.grid(row=1,column=3)
                self.sortColumnsSampleCombobox.grid(row=1,column=4)
                
                self.sortColumnsBackButton.grid(row=0,column=0)
                self.sortColumnsSkipButton.grid(row=0,column=1)
                self.sortColumnsNextButton.grid(row=0,column=2,padx=10)
                
                self.sortColumnsWin.bind('<Key>',sortKeyPress)
        ...
        for index in self.selected:
            for n in range(len(self.selectReplaceEntries)):
                ...
                if len(text) < 1 or replaceValue not in str(self.df.loc[index, col]).lower():
                    continue
                if col in [self.columns[3], self.columns[7], self.columns[6]]:
                    try:
                        text = pd.to_datetime(text)
                    except:
                        text = float('nan')
                elif col == self.columns[8]:
                    try:
                        text = float(text)
                    except:
                        text = float('nan')
                elif col == self.columns[4]:
    ...
    def search(self, update=True, resetPage=True, performSearch=True):
        ...
        try:
            ...
            for row in range(len(self.results[self.page])):
                for n in range(len(self.resultEntries[row])):
    ...
    def confirmAdd(self):
        ...
        self.df = self.df.append(entry, ignore_index=False)
        self.df.to_csv(self.fileName)
    ...
    def addSuggest(self, i=None):
        ...
        if type(self.focus) == Entry:
            entry, sortedDf = self.focus.get(), self.df.sort_values(
                self.columns[3], ascending=True).copy()[-50:]
            ...
            if len(entry) == 0:
                ...
                if self.focus == self.addTitle:
                    ...
                    )[-15:], key=sortedDf[self.columns[0]][-15:].to_list().count)
                ...
                results = self.print_list(
                    str(entry), False, 3, list(self.df[col].unique()))
                results.reverse()
    ...
    def tutorial(self):
        if self.curState=='fileNotFound':
            ...
            t1 = msg.showinfo(master=self.mainWin, title='Welcome!',
                              message=f'View, edit and search through all your {self.itemName}s within all one app!\n\nJust go through one small step to get started.')
        if t1 == 'ok':
            t2=msg.askyesno(master=self.mainWin,title=f'Start from an empty {self.systemName}?',message='Would you like to start with an empty {self.systemName}?\n(Press "No" if you have a data file)')
            
            if t2==True:
                self.newDf(True,True)
            else:
                self.openDf()
            #t2=msg.showinfo(title='Other Information',master=self.mainWin,message='Press the "Add Book" button to add books to the catlog\n\nTap the "i" button for a summary of the books, authors, donors and types of books in the catalog.\nPress CTRL+/ to instantly jump to the search bar.')
    ...
    def pageWinDeiconify(self):
        self.pageWin.geometry(f'+615+{self.pageFrame.winfo_rooty()+55}')
    ...
    def print_list(self, search=None, returnIndex=False, numValues=20, data=None):

        if type(data) == type(None):
            data = dict()
            for index in self.df.index:
                data[index] = str(' ').join([str(item)
